...
Ingests 5 years of daily OHLCV bars for 50 equities into AWS S3 Data Lake.
"""
...
def main() -> None:
    ...
    if not all_frames:
        print("\n[ERROR] No data retrieved. Check your API keys.")
        sys.exit(1)

    combined_df = pd.concat(all_frames, ignore_index=True)
    combined_df.columns = [c.lower() for c in combined_df.columns]
    combined_df["ingestion_timestamp"] = datetime.now(timezone.utc).isoformat()
    ...
    print(f"[ERROR] Failed tickers     : {failed if failed else 'None'}")
    print(f"📅 Date range         : {combined_df['timestamp'].min()} -> {combined_df['timestamp'].max()}")
    print(f"📦 Total rows         : {len(combined_df):,}")
    print(f"📦 AWS Storage path : {get_s3_path('stocks')}")
    print("=" * 60)
